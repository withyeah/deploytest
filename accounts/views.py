...
def people(request, username):
    ...
    followers = people.followers.all()
    followings = people.followings.all()
    
    recommend_movies = Movie.objects.filter().all()[:10]
    context = {
            'people':people,
    ...
    if followers:
        context['follower_movies'] = Comment.objects.filter(user__in=followers).order_by('-score')[:10]
        
    if followings:
        context['following_movies'] = Comment.objects.filter(user__in=followings).order_by('-score')[:10]
        
    
    
    # 추천
    user_comment = Comment.objects.filter(user=people).order_by('-score').values_list('movie', flat=True).distinct()[:1]
    if user_comment:
        movie_genre = Movie.objects.filter(pk=user_comment[0]).order_by('pk').values_list('genre', flat=True).distinct()
        context['recommend_movies'] = Movie.objects.filter(genre=movie_genre[0]).all()[:10]
        context['movie_genre'] = Genre.objects.filter(pk=movie_genre[0]).all()[0]
